...
class Test {
    ...
    static private boolean doGet(KaskaClient cl, Scanner ent) throws RemoteException {
        ...
        if (r == null)
            ...
            System.err.println("Mensaje recibido: tema " + r.getTopic() + " offset " + r.getOffset() + " contenido " + (m = r.getMessage()) + " " + m.getClass());
    ...
    static private void prompt() {
        ...
        System.err.println("\toperaciones: createTopics|topicList|sendString|SendClass|get|subscribe|unsubscribe|position|seek|poll");
    }
    static private boolean doSendClass(KaskaClient cl, Scanner ent) throws RemoteException {
        System.err.println("Introduzca el nombre del tema y el nombre completo de la clase (paquete.clase) de la que quiere enviar un objeto: ");
        if (!ent.hasNextLine()) return false;
        String lin = ent.nextLine();
        Scanner s = new Scanner(lin);
        if  (!s.hasNext()) return false;
        ...
        try {
            ...
            while (true) {
                ...
                if (s.hasNext()) {
                   String op = s.next();
                   switch (op) {
                       case "createTopics": formatoOK = doCreateTopics(cl, ent); break;
                       case "topicList": formatoOK = doTopicList(cl, ent); break;
                       case "sendString": formatoOK = doSendString(cl, ent); break;
                       case "sendClass": formatoOK = doSendClass(cl, ent); break;
                       case "get": formatoOK = doGet(cl, ent); break;
                       case "subscribe": formatoOK = doSubscribe(cl, ent); break;
                       case "unsubscribe": formatoOK = doUnsubscribe(cl, ent); break;
                       case "position": formatoOK = doPosition(cl, ent); break;
                       case "seek": formatoOK = doSeek(cl, ent); break;
                       case "poll": formatoOK = doPoll(cl, ent); break;
                    }
                }
                if (!formatoOK)
